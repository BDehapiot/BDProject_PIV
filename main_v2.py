...
# stack_name = '18-07-11_40x_GBE_UtrCH_Ctrl_b1_uint8_short.tif'
...
for t in range(1, nT):       
    for y in range(nIntY):
        for x in range(nIntX):
            ...
            u[t,y,x] = x_max - (srcSize-1)
            v[t,y,x] = y_max - (srcSize-1)

end = time.time()
print(f'  {(end-start):5.3f} s') 

srch = srcWin - np.mean(srcWin)
intr = intWin - np.mean(intWin)

import cv2

test = cv2.matchTemplate(srch.astype('float32'),intr.astype('float32'),method=2)
